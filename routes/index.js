...
const { getUnspent,getUnspentForTX,getUnspentTotalAmount, getLatestBlockHash,getBlockHash, getBlockHeight,determinePayFee, checkAmountBigthanFee,pushTX } = require('../services/zencash');
/* GET home page. */
...
router.get('/sendTX',async (req, res, next)=>{
  try{
    ...
    const receiverArray=[{address: 'znb9Nn6z7JR8RNPzVd5UJJadcp2ji8dAG9W', satoshis: sendAmount},{address: 'znnA2VibNgeJfq41B44dWjXiGnv7GdAV2Lb', satoshis: returnAmount}]
    if(couldTX){
      var txobj= zencashjs.transaction.createRawTx(unspentForTXArray,receiverArray,blockheightForTX,blockhashForTX);
      var signedobj='';
      let j=0;
      // Remove prevScriptPubKey since it's not really an attribute
      for (var i = 0; i < txobj.ins.length; i++) {
      ...
      var signed_serialized = zencashjs.transaction.serializeTx(signedobj);
      console.log('signed_serialized',signed_serialized)
